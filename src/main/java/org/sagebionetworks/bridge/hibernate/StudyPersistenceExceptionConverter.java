...
@Component
public class StudyPersistenceExceptionConverter implements PersistenceExceptionConverter {

    @Override
    public RuntimeException convert(PersistenceException exception, Object entity) {
        if (exception instanceof OptimisticLockException) {
            return new ConcurrentModificationException((HibernateStudy)entity);
        }
        if (exception.getCause() instanceof org.hibernate.exception.ConstraintViolationException) {
            ...
            String finalMessage = "Study table constraint prevented save or update.";
            if (message.matches(".*a foreign key constraint fails.*REFERENCES `Substudies`.*")) {
                finalMessage = "Study cannot be deleted, it is referenced by an account";
            }
