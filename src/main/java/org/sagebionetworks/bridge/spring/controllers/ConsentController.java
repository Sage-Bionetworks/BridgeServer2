...
public class ConsentController extends BaseController {
    ...
    public StatusMessage emailCopy(@RequestBody(required=false) String body) throws Exception {
        final UserSession session = getAuthenticatedAndConsentedSession();
        
        return resendConsentAgreement(session.getStudyIdentifier().getIdentifier(), null);
    }
    ...
    public JsonNode withdrawConsentV2(@PathVariable String guid) throws Exception {
        final UserSession session = getAuthenticatedSession();
        final Withdrawal withdrawal = parseJson(Withdrawal.class);
        final Study study = studyService.getStudy(session.getStudyIdentifier());
    ...
    public StatusMessage resendConsentAgreement(@PathVariable String guid, @RequestBody(required=false) String body) {
        final UserSession session = getAuthenticatedAndConsentedSession();
        final Study study = studyService.getStudy(session.getStudyIdentifier());
    ...
    private JsonNode giveConsentForVersion(int version, SubpopulationGuid subpopGuid) throws Exception {
        ...
        final ConsentSignature consentSignature = ConsentSignature.fromJSON(node);
