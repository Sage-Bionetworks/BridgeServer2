...
public class FileControllerTest extends Mockito {
    ...
    public void getFilesIncludeDeleted() {
        ...
        when(mockFileService.getFiles(TEST_STUDY_IDENTIFIER, 5, 40, true)).thenReturn(page);
        
        ResourceList<FileMetadata> results = controller.getFiles("5", "40", "true");
        assertSame(results, page);
        
        verify(mockFileService).getFiles(TEST_STUDY_IDENTIFIER, 5, 40, true);
    }
    
    @Test
    public void getFilesExcludeDeleted() {
        PagedResourceList<FileMetadata> page = new PagedResourceList<>(ImmutableList.of(new FileMetadata()), 100);
        when(mockFileService.getFiles(TEST_STUDY_IDENTIFIER, 0, API_DEFAULT_PAGE_SIZE, false)).thenReturn(page);
        
        ResourceList<FileMetadata> results = controller.getFiles(null, null, "false");
        assertSame(results, page);
        
        verify(mockFileService).getFiles(TEST_STUDY_IDENTIFIER, 0, API_DEFAULT_PAGE_SIZE, false);
    }
    ...
    public void createFile() throws Exception {
        ...
        when(mockFileService.createFile(eq(TEST_STUDY_IDENTIFIER), any())).thenReturn(persisted);
        ...
        verify(mockFileService).createFile(eq(TEST_STUDY_IDENTIFIER), metadataCaptor.capture());
    ...
    public void getFile() {
        FileMetadata persisted = new FileMetadata();
        when(mockFileService.getFile(TEST_STUDY_IDENTIFIER, GUID)).thenReturn(persisted);
        
        FileMetadata file = controller.getFile(GUID);
        assertSame(file, persisted);
        
        verify(mockFileService).getFile(TEST_STUDY_IDENTIFIER, GUID);
    }
    ...
    public void updateFile() throws Exception {
        ...
        persisted.setGuid(GUID);
        persisted.setVersion(3L);
        when(mockFileService.updateFile(eq(TEST_STUDY_IDENTIFIER), any())).thenReturn(persisted);
        ...
        verify(mockFileService).updateFile(eq(TEST_STUDY_IDENTIFIER), metadataCaptor.capture());
        assertEquals(metadataCaptor.getValue().getName(), "a test name");
    ...
    public void deleteFileDefault() throws Exception {
        ...
        verify(mockFileService).deleteFile(TEST_STUDY_IDENTIFIER, GUID);
    }
    ...
    public void adminCanPermanentlyDelete() throws Exception {
        ...
        verify(mockFileService).deleteFilePermanently(TEST_STUDY_IDENTIFIER, GUID);
    }
    ...
    public void createFileRevision() throws Exception {
        ...
        verify(mockFileService).createFileRevision(eq(TEST_STUDY_IDENTIFIER), revisionCaptor.capture());
    ...
    public void finishFileRevision() throws Exception {
        ...
        verify(mockFileService).finishFileRevision(eq(TEST_STUDY_IDENTIFIER), eq(GUID), dateTimeCaptor.capture());
    ...
    public void getFileRevisions() throws Exception {
        ...
        when(mockFileService.getFileRevisions(TEST_STUDY_IDENTIFIER, GUID, 10, 50)).thenReturn(list);
        
        PagedResourceList<FileRevision> revisions = controller.getFileRevisions(GUID, "10", "50");
        assertSame(revisions, list);
        
        verify(mockFileService).getFileRevisions(TEST_STUDY_IDENTIFIER, GUID, 10, 50);
    }
    ...
    public void getFileRevisionsDefaults() { 
        ...
        verify(mockFileService).getFileRevisions(TEST_STUDY_IDENTIFIER, GUID, 0, API_DEFAULT_PAGE_SIZE);
    }
}
