...
public class FileServiceTest extends Mockito {
    ...
    public void getFilesExcludeDeleted() {
        service.getFiles(API_APP_ID, 5, 30, false);
        
        verify(mockFileDao).getFiles(API_APP_ID, 5, 30, false);
    }

    @Test
    public void getFilesIncludeDeleted() {
        service.getFiles(API_APP_ID, 0, 25, true);
        
        verify(mockFileDao).getFiles(API_APP_ID, 0, 25, true);
    }

    @Test(expectedExceptions = BadRequestException.class)
    public void getFilesPageSizeTooSmall() {
        service.getFiles(API_APP_ID, 0, 1, false);
    }
    
    @Test(expectedExceptions = BadRequestException.class)
    public void getFilesPageSizeTooLarge() {
        service.getFiles(API_APP_ID, 0, 1000, false);
    }
    
    @Test
    public void getFile() {
        FileMetadata metadata = new FileMetadata();
        when(mockFileDao.getFile(API_APP_ID, GUID)).thenReturn(Optional.of(metadata));
        
        FileMetadata returned = service.getFile(API_APP_ID, GUID);
        assertSame(returned, metadata);
        
        verify(mockFileDao).getFile(API_APP_ID, GUID);
    }
    
    @Test(expectedExceptions = EntityNotFoundException.class)
    public void getFileNotFound() { 
        when(mockFileDao.getFile(API_APP_ID, GUID)).thenReturn(Optional.empty());
        
        service.getFile(API_APP_ID, GUID);
    }
    ...
    public void createFile() {
        ...
        FileMetadata returned = service.createFile(API_APP_ID, metadata);
    ...
    public void createFileValidated() {
        FileMetadata metadata = new FileMetadata();
        service.createFile(API_APP_ID, metadata);
    }
    ...
    public void updateFile() {
        ...
        when(mockFileDao.getFile(API_APP_ID, GUID)).thenReturn(Optional.of(persisted));
        when(mockFileDao.updateFile(any())).thenReturn(persisted);
        
        FileMetadata metadata = new FileMetadata();
        metadata.setName(NAME);
        metadata.setGuid(GUID);
        FileMetadata returned = service.updateFile(API_APP_ID, metadata);
    ...
    public void updateFileValidated() {
        ...
        FileMetadata metadata = new FileMetadata();
        metadata.setGuid(GUID);
        service.updateFile(API_APP_ID, metadata);
    }
    ...
    public void deleteFile() {
        ...
        service.deleteFile(API_APP_ID, GUID);
    ...
    public void deleteFilePermanently() {
        service.deleteFilePermanently(API_APP_ID, GUID);
        
        verify(mockFileDao).deleteFilePermanently(API_APP_ID, GUID);
    }
    
    @Test
    public void deleteAllStudyFiles() {
        service.deleteAllStudyFiles(API_APP_ID);
        
        verify(mockFileDao).deleteAllStudyFiles(API_APP_ID);
    }
    
    @Test
    public void getFileRevisions() {
        FileMetadata metadata = new FileMetadata();
        doReturn(metadata).when(service).getFile(API_APP_ID, GUID);
        ...
        ResourceList<FileRevision> captured = service.getFileRevisions(API_APP_ID,GUID, 100, 25);
    ...
    public void getFileRevisionsPageSizeTooSmall() {
        ...
        service.getFileRevisions(API_APP_ID,GUID, 0, 1);
    }
    ...
    public void getFileRevisionsSizeTooLarge() {
        ...
        service.getFileRevisions(API_APP_ID, GUID, 0, 1000);
    }
    ...
    public void createFileRevision() throws Exception {
        ...
        FileRevision returned = service.createFileRevision(API_APP_ID, revision);
    ...
    public void createFileRevisionNotFound() throws Exception {
        ...
        service.createFileRevision(API_APP_ID, revision);
    }
    ...
    public void finishFileRevision() {
        ...
        service.finishFileRevision(API_APP_ID, GUID, TIMESTAMP);
