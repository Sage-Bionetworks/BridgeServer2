...
public class SurveyServiceMockTest {
    ...
    public void createSurvey_IdentifierAlreadyExists() {
        ...
        when(mockSurveyDao.getSurveyGuidForIdentifier(API_APP_ID, SURVEY_ID)).thenReturn(SURVEY_GUID);
    ...
    public void getSurveyWithoutElements() {
        ...
        service.getSurvey(API_APP_ID, SURVEY_KEYS, false, true);
        
        verify(mockSurveyDao).getSurvey(API_APP_ID, SURVEY_KEYS, false);
    }
    ...
    public void getSurveyMostRecentlyPublishedWithoutElements() {
        Survey survey = Survey.create();
        survey.setStudyIdentifier(API_APP_ID);
        when(mockSurveyDao.getSurveyMostRecentlyPublishedVersion(API_APP_ID, SURVEY_GUID, false))
                .thenReturn(survey);
        
        service.getSurveyMostRecentlyPublishedVersion(API_APP_ID, SURVEY_GUID, false);
        
        verify(mockSurveyDao).getSurveyMostRecentlyPublishedVersion(API_APP_ID, SURVEY_GUID, false);
    }
    
    @Test
    public void getAllSurveysMostRecentlyPublishedVersionIncludeDeleted() {
        service.getAllSurveysMostRecentlyPublishedVersion(API_APP_ID, true);
        
        verify(mockSurveyDao).getAllSurveysMostRecentlyPublishedVersion(API_APP_ID, true);
    }
    
    @Test
    public void getAllSurveysMostRecentlyPublishedVersionExcludeDeleted() {
        service.getAllSurveysMostRecentlyPublishedVersion(API_APP_ID, false);
        
        verify(mockSurveyDao).getAllSurveysMostRecentlyPublishedVersion(API_APP_ID, false);
    }
    
    @Test
    public void getAllSurveysMostRecentVersionIncludeDeleted() {
        service.getAllSurveysMostRecentVersion(API_APP_ID, true);
        
        verify(mockSurveyDao).getAllSurveysMostRecentVersion(API_APP_ID, true);
    }
    
    @Test
    public void getAllSurveysMostRecentVersionExcludeDeleted() {
        service.getAllSurveysMostRecentVersion(API_APP_ID, false);
        
        verify(mockSurveyDao).getAllSurveysMostRecentVersion(API_APP_ID, false);
    }
    ...
    public void checkConstraintsBeforePhysicalDelete() {
        ...
        when(mockSurveyDao.getSurvey(API_APP_ID, surveyKeys, false)).thenReturn(existing);
        
        // Now create a schedule that points to this survey
        List<SchedulePlan> plans = createSchedulePlanListWithSurveyReference(false);
        when(mockSchedulePlanService.getSchedulePlans(ClientInfo.UNKNOWN_CLIENT, API_APP_ID, true)).thenReturn(plans);
        
        service.deleteSurveyPermanently(API_APP_ID, surveyKeys);
    }
    ...
    public void publishSurvey() {
        ...
        // mock DAO
        when(mockSurveyDao.getSurvey(API_APP_ID, SURVEY_KEYS, true)).thenReturn(survey);
        when(mockSurveyDao.publishSurvey(API_APP_ID, survey, true)).thenReturn(survey);

        // mock publish validator
        when(mockSurveyPublishValidator.supports(any())).thenReturn(true);

        // execute and validate
        Survey retval = service.publishSurvey(API_APP_ID, SURVEY_KEYS, true);
    ...
    public void publishSurveyDeleted() {
        ...
        service.publishSurvey(API_APP_ID, SURVEY_KEYS, true);
    }
    
    @Test(expectedExceptions = EntityNotFoundException.class)
    public void publishSurveyDoesNotExist() {
        when(mockSurveyDao.getSurvey(API_APP_ID, SURVEY_KEYS, true)).thenReturn(null);
    ...
    public void successfulDelete() {
        ...
        service.deleteSurvey(API_APP_ID, survey);
    ...
    public void successfulDeletePermanently() {
        ...
        doReturn(plans).when(mockSchedulePlanService).getSchedulePlans(ClientInfo.UNKNOWN_CLIENT, API_APP_ID, true);
        Survey survey = createSurvey();
        when(mockSurveyDao.getSurvey(any(), any(), eq(false))).thenReturn(survey);
        
        service.deleteSurveyPermanently(API_APP_ID, survey);
        ...
        verify(mockSurveyDao).deleteSurveyPermanently(eq(API_APP_ID), keysCaptor.capture());
    ...
    public void physicallyDeleteSurveyNotEmptySharedModules() {
        ...
        when(mockSchedulePlanService.getSchedulePlans(ClientInfo.UNKNOWN_CLIENT, API_APP_ID, true))
    ...
    public void deleteSurveyFailsOnMissingSurvey() {
        ...
        try {
            ...
            service.deleteSurvey(API_APP_ID, SURVEY_KEYS);
            fail("Should have thrown exception");
        } catch(EntityNotFoundException e) {
            verify(mockSurveyDao, never()).deleteSurvey(any());
        }
    ...
    public void deleteSurveyPermanentlyConstrainedByScheduleWithPublishedSurvey() {
        ...
        doReturn(Lists.newArrayList(unpubSurvey, survey)).when(mockSurveyDao).getSurveyAllVersions(API_APP_ID, SURVEY_GUID, false);
        when(mockSurveyDao.getSurvey(any(), any(), eq(false))).thenReturn(survey);
    ...
    public void deleteSurveyPermanentlyWithNoOlderPublishedVersionsConstrainedByScheduleWithPublishedSurvey() {
        ...
        survey.setPublished(true);
        when(mockSurveyDao.getSurvey(any(), any(), eq(false))).thenReturn(survey);
        doReturn(Lists.newArrayList(survey)).when(mockSurveyDao).getSurveyAllVersions(API_APP_ID, SURVEY_GUID, false);
    ...
    public void deleteSurveyPermanentlyWithOlderPublishedVersionOK() {
        ...
        doReturn(Lists.newArrayList(survey1, survey2)).when(mockSurveyDao).getSurveyAllVersions(API_APP_ID, SURVEY_GUID, false);
        
        //Does not throw an exception
        service.deleteSurveyPermanently(API_APP_ID, survey1);
    }
    ...
    public void deleteSurveyPermanentlyNotConstrainedByCompoundScheduleWithMultiplePublishedSurveys() {
        // Two published surveys in the list, no exception thrown
        Survey survey = createSurvey();
        when(mockSurveyDao.getSurvey(API_APP_ID, survey, false)).thenReturn(survey);
        doReturn(Lists.newArrayList(survey)).when(mockSurveyDao).getSurveyAllVersions(API_APP_ID, survey.getGuid(), false);
        List<SchedulePlan> plans = createSchedulePlanListWithCompoundActivity(true);
        doReturn(plans).when(mockSchedulePlanService).getSchedulePlans(ClientInfo.UNKNOWN_CLIENT, API_APP_ID, true);
    ...
    public void updateSurveyFailsOnDeletedSurvey() throws Exception {
        ...
        service.updateSurvey(API_APP_ID, update);
    }
    ...
    public void updateSurveyUndeletePublishedOK() {
        ...
        verify(mockSurveyDao).updateSurvey(eq(API_APP_ID), surveyCaptor.capture());
    ...
    public void versionSurveyFailsOnDeletedSurvey() throws Exception {
        ...
        service.versionSurvey(API_APP_ID, SURVEY_KEYS);
    }
    ...
    public void deleteSurveyPermanentlyFailsOnMissingSurvey() {
        ...
        service.deleteSurveyPermanently(API_APP_ID, SURVEY_KEYS);
    }
    
    @Test(expectedExceptions = EntityNotFoundException.class)
    public void getSurveyAllVersionsThrowsException() {
        service.getSurveyAllVersions(API_APP_ID, "GUID", true);
    }
    ...
    public void getSurveyInOtherStudyThrowsException() {
        Survey survey = Survey.create();
        survey.setStudyIdentifier(API_APP_ID);
        when(mockSurveyDao.getSurvey(API_APP_ID, SURVEY_KEYS, false)).thenReturn(survey);
    ...
    public void getSurveyReturnsNull() {
        assertNull(service.getSurvey(API_APP_ID, SURVEY_KEYS, false, false));
    ...
    public void updateSurveyInOtherStudy() {
        ...
        survey.setStudyIdentifier(API_APP_ID);
        when(mockSurveyDao.getSurvey(API_APP_ID, (GuidCreatedOnVersionHolder)survey, false)).thenReturn(survey);
    ...
    public void publishSurveyInOtherStudy() {
        ...
        survey.setCreatedOn(SURVEY_CREATED_ON.getMillis());
        survey.setStudyIdentifier(API_APP_ID);
        when(mockSurveyDao.getSurvey(API_APP_ID, (GuidCreatedOnVersionHolder)survey, true)).thenReturn(survey);
    ...
    public void versionSurveyInOtherStudy() {
        ...
        verify(mockSurveyDao, never()).versionSurvey(API_APP_ID, SURVEY_KEYS);
    }
    ...
    public void getSurveyAllVersionsIncludeDeleted() {
        ...
        when(mockSurveyDao.getSurveyAllVersions(API_APP_ID, SURVEY_GUID, true)).thenReturn(list);
        
        List<Survey> results = service.getSurveyAllVersions(API_APP_ID, SURVEY_GUID, true);
        assertEquals(results.size(), 2);
        
        verify(mockSurveyDao).getSurveyAllVersions(API_APP_ID, SURVEY_GUID, true);
    }
    
    @Test
    public void getSurveyAllVersionsExcludeDeleted() {
        List<Survey> list = ImmutableList.of(Survey.create(), Survey.create());
        when(mockSurveyDao.getSurveyAllVersions(API_APP_ID, SURVEY_GUID, false)).thenReturn(list);
        
        List<Survey> results = service.getSurveyAllVersions(API_APP_ID, SURVEY_GUID, false);
        assertEquals(results.size(), 2);
        
        verify(mockSurveyDao).getSurveyAllVersions(API_APP_ID, SURVEY_GUID, false);
    }
    
    @Test(expectedExceptions = EntityNotFoundException.class)
    public void getSurveyAllVersionsNotFound() {
        service.getSurveyAllVersions(API_APP_ID, SURVEY_GUID, false);
    }
