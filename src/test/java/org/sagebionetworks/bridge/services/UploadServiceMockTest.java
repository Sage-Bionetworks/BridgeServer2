...
public class UploadServiceMockTest {
    ...
    private void setupUploadMocks() {
        ...
        ForwardCursorPagedResourceList<Upload> pagedList = new ForwardCursorPagedResourceList<>(results, MOCK_OFFSET_KEY)
            .withRequestParam(ResourceList.PAGE_SIZE, API_MAXIMUM_PAGE_SIZE);
        doReturn(pagedList).when(mockUploadDao).getStudyUploads(TEST_STUDY_IDENTIFIER, START_TIME, END_TIME, API_MAXIMUM_PAGE_SIZE, MOCK_OFFSET_KEY);
        doReturn(pagedList).when(mockUploadDao).getStudyUploads(TEST_STUDY_IDENTIFIER, START_TIME, END_TIME, API_DEFAULT_PAGE_SIZE, null);
    ...
    public void canGetStudyUploads() throws Exception {
        ...
        ForwardCursorPagedResourceList<UploadView> returned = svc.getStudyUploads(TEST_STUDY_IDENTIFIER,
                START_TIME, END_TIME, API_MAXIMUM_PAGE_SIZE, MOCK_OFFSET_KEY);
        
        verify(mockUploadDao).getStudyUploads(TEST_STUDY_IDENTIFIER, START_TIME, END_TIME, API_MAXIMUM_PAGE_SIZE, MOCK_OFFSET_KEY);
    ...
    public void canGetStudyUploadsWithoutPageSize() throws Exception {
        ...
        svc.getStudyUploads(TEST_STUDY_IDENTIFIER, START_TIME, END_TIME, null, null);

        verify(mockUploadDao).getStudyUploads(TEST_STUDY_IDENTIFIER, START_TIME, END_TIME, API_DEFAULT_PAGE_SIZE, null);
    }
    ...
    public void createUpload() throws Exception {
        ...
        when(mockUploadDao.createUpload(uploadRequest, TEST_STUDY_IDENTIFIER, HEALTH_CODE, null)).thenReturn(upload);
        ...
        UploadSession session = svc.createUpload(TEST_STUDY_IDENTIFIER, PARTICIPANT, uploadRequest);
    ...
    public void createUploadDuplicate() throws Exception {
        ...
        when(mockUploadDao.getUpload(ORIGINAL_UPLOAD_ID)).thenReturn(upload);
        when(mockUploadDao.createUpload(uploadRequest, TEST_STUDY_IDENTIFIER, HEALTH_CODE, ORIGINAL_UPLOAD_ID)).thenReturn(upload);
        when(mockUploadCredentailsService.getSessionCredentials())
                .thenReturn(new BasicSessionCredentials(null, null, null));
        when(mockS3UploadClient.generatePresignedUrl(any())).thenReturn(new URL("https://ws.com/some-link"));
    ...
    public void uploadComplete() throws Exception {
        ...
        svc.uploadComplete(TEST_STUDY_IDENTIFIER, S3_WORKER, upload, true);
    ...
    public void uploadCompleteCannotBeValidated() throws Exception {
        ...
        svc.uploadComplete(TEST_STUDY_IDENTIFIER, S3_WORKER, upload, false);
    ...
    public void uploadCompleteConcurrentModificationException() throws Exception {
        ...
        verify(mockUploadDao).uploadComplete(S3_WORKER, upload);
        verify(mockUploadValidationService, never()).validateUpload(TEST_STUDY_IDENTIFIER, upload);
    }
