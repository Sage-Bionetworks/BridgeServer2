...
public class TemplateServiceTest extends Mockito {
    ...
    public void beforeMethod() throws Exception {
        ...
        when(mockSubstudyService.getSubstudyIds(TEST_STUDY_IDENTIFIER)).thenReturn(USER_SUBSTUDY_IDS);
    }
    ...
    private Template makeTemplate(String guid, String lang) {
        ...
        when(mockTemplateDao.getTemplate(TEST_STUDY_IDENTIFIER, guid)).thenReturn(Optional.of(template));
        return template;
    }
    
    private CriteriaContext makeContext(String lang) {
        return new CriteriaContext.Builder().withStudyIdentifier(TEST_STUDY_IDENTIFIER).withLanguages(ImmutableList.of(lang))
                .build();
    }
    ...
    private void mockGetTemplates(List<? extends Template> list) {
        ...
        doReturn(page).when(mockTemplateDao).getTemplates(TEST_STUDY_IDENTIFIER, EMAIL_RESET_PASSWORD, null, null, false);
    }
    ...
    public void getTemplatesForType() {
        ...
        doReturn(resourceList).when(mockTemplateDao).getTemplates(TEST_STUDY_IDENTIFIER, EMAIL_RESET_PASSWORD, 5, 50, true);
        
        Criteria criteria = Criteria.create();
        when(mockCriteriaDao.getCriteria(any())).thenReturn(criteria);
        
        PagedResourceList<? extends Template> results = service.getTemplatesForType(TEST_STUDY_IDENTIFIER, EMAIL_RESET_PASSWORD, 5, 50, true);
        assertSame(results, resourceList);
        
        for (Template template : results.getItems()) {
            assertNotNull(template.getCriteria());
        }
        verify(mockTemplateDao).getTemplates(TEST_STUDY_IDENTIFIER, EMAIL_RESET_PASSWORD, 5, 50, true);
    ...
    public void getTemplatesForTypeDefaultsOffset() {
        ...
        doReturn(resourceList).when(mockTemplateDao).getTemplates(TEST_STUDY_IDENTIFIER, EMAIL_RESET_PASSWORD, 0, 50, true);
        
        service.getTemplatesForType(TEST_STUDY_IDENTIFIER, EMAIL_RESET_PASSWORD, null, 50, true);
        
        verify(mockTemplateDao).getTemplates(TEST_STUDY_IDENTIFIER, EMAIL_RESET_PASSWORD, 0, 50, true);
    }

    @Test
    public void getTemplatesForTypeDefaultsPageSize() {
        PagedResourceList<? extends Template> resourceList = new PagedResourceList<>(ImmutableList.of(), 150);
        doReturn(resourceList).when(mockTemplateDao).getTemplates(TEST_STUDY_IDENTIFIER, EMAIL_RESET_PASSWORD, 5, API_DEFAULT_PAGE_SIZE, true);
        
        service.getTemplatesForType(TEST_STUDY_IDENTIFIER, EMAIL_RESET_PASSWORD, 5, null, true);
        
        verify(mockTemplateDao).getTemplates(TEST_STUDY_IDENTIFIER, EMAIL_RESET_PASSWORD, 5, API_DEFAULT_PAGE_SIZE, true);
    }
    ...
    public void getTemplatesPageSizeBelowMin() {
        service.getTemplatesForType(TEST_STUDY_IDENTIFIER, EMAIL_RESET_PASSWORD, null, 0, true);
    }

    @Test(expectedExceptions = BadRequestException.class)
    public void getTemplatesPageSizeAboveMax() {
        service.getTemplatesForType(TEST_STUDY_IDENTIFIER, EMAIL_RESET_PASSWORD, null, 1000, true);
    }
    ...
    public void getTemplate() {
        ...
        when(mockTemplateDao.getTemplate(TEST_STUDY_IDENTIFIER, GUID1)).thenReturn(Optional.of(template));
        
        Template result = service.getTemplate(TEST_STUDY_IDENTIFIER, GUID1);
    ...
    public void getTemplateNoGuid() {
        service.getTemplate(TEST_STUDY_IDENTIFIER, null);
    }

    @Test(expectedExceptions = EntityNotFoundException.class)
    public void getTemplateNotFound() {
        when(mockTemplateDao.getTemplate(TEST_STUDY_IDENTIFIER, GUID1)).thenReturn(Optional.empty());
        
        service.getTemplate(TEST_STUDY_IDENTIFIER, GUID1);
    }
    ...
    public void updateTemplate() {
        ...
        when(mockTemplateDao.getTemplate(TEST_STUDY_IDENTIFIER, GUID1)).thenReturn(Optional.of(existing));
        ...
        GuidVersionHolder result = service.updateTemplate(TEST_STUDY_IDENTIFIER, template);
    ...
    public void updateTemplateSucceedsWithDefaultIfNotDeleting() {
        ...
        service.updateTemplate(TEST_STUDY_IDENTIFIER, template);
    ...
    public void deleteTemplate() {
        ...
        service.deleteTemplate(TEST_STUDY_IDENTIFIER, GUID1);
    ...
    public void deleteTemplatePermanently() {
        ...
        service.deleteTemplatePermanently(TEST_STUDY_IDENTIFIER, GUID1);

        verify(mockCriteriaDao).deleteCriteria("template:"+GUID1);
        verify(mockTemplateDao).deleteTemplatePermanently(TEST_STUDY_IDENTIFIER, GUID1);
    }
    ...
    public void deleteTemplatesForStudy() {
        service.deleteTemplatesForStudy(TEST_STUDY_IDENTIFIER);
        
        verify(mockTemplateDao).deleteTemplatesForStudy(TEST_STUDY_IDENTIFIER);
    }
}
